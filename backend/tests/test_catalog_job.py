...
Requires Docker. Spins up and tears down its own container --
`pytest backend/tests/test_catalog_job.py` is self-contained, no manual
setup needed. NEVER points at the real Supabase project -- see
...
from nimtracker import catalog_scraper, db
...
def conn(pg_dsn):
    ...
    run_catalog_job(), which commits internally (it's meant to be one
    atomic unit of work), so a rollback-based isolation pattern doesn't
    work here; truncate-before is simpler and uniform for every test.
    """
...
def _seed_model(conn, **overrides):
    row = {
...
def test_upsert_model_sets_first_discovered_at_only_on_insert(conn):
    _seed_model(conn, slug="acme/a", catalog_href="/acme/a")
    ...
    _seed_model(conn, slug="acme/a", catalog_href="/acme/a", model_name="widget-9001")
    with conn.cursor() as cur:
...
def test_upsert_model_does_not_touch_lifecycle_fields(conn):
    _seed_model(conn, slug="acme/b", catalog_href="/acme/b")
    with conn.cursor() as cur:
        cur.execute(
    ...
    _seed_model(conn, slug="acme/b", catalog_href="/acme/b", model_name="widget-9002")
...
def test_run_catalog_job_list_scrape_failure_uses_db_fallback(conn, monkeypatch):
    _seed_model(conn, slug="acme/a", catalog_href="/acme/a")
    _seed_model(conn, slug="acme/b", catalog_href="/acme/b")
...
def test_run_catalog_job_detail_scrape_failure_falls_back_to_existing_row(conn, monkeypatch):
    _seed_model(
        conn,
        slug="acme/widget-9000",
        catalog_href="/acme/widget-9000",
...
def test_run_catalog_job_redirecting_href_finds_existing_row_via_catalog_href(conn, monkeypatch):
